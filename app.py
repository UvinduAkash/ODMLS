...
def dashboard():
    ...
    # Fetch users to display in dashboard
    connection = get_db_connection()
    if not connection:
    ...
    try:
        cursor = connection.cursor(dictionary=True)
        query = """
            SELECT user_id, first_name, last_name, username, role, email
            FROM users
        """
        cursor.execute(query)
    ...
    # Pass the user's role to the template for conditional rendering
...
def add_user():
    ...
    if not (first_name and last_name and user_role and username and password):
    ...
    try:
        ...
        query = """
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """
        email = f"{username}@ousl.lk"  # Assuming email is generated from username
        cursor.execute(query, (username, password, email, user_role, first_name, last_name))
        connection.commit()
        flash("User account created successfully!")
    except mysql.connector.Error as err:
        print(f"Error adding user: {err}")
        flash("An error occurred while creating the user account.")
    finally:
        cursor.close()
        connection.close()
